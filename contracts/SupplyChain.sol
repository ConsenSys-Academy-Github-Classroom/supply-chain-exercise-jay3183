...
modifier isBuyer(uint sku){
  require(items[sku].buyer == msg.sender, "Caller is not buyer");
  _;
}
modifier isSeller(uint sku){
   require(items[sku].seller == msg.sender, "Caller is not seller");
  _;
}
  ...
  // an Item has been added?
   ...
   modifier forSale(uint _sku){
    require(items[_sku].sku >= 0,"Item is not for sale");
     _;
   } 
  ...
  function shipItem(uint sku) public sold(sku) verifyCaller(msg.sender) isSeller(sku){
  ...
  // 3. Call the event associated with this function!
  function receiveItem(uint sku) public shipped(sku) verifyCaller(msg.sender) isBuyer(sku){
